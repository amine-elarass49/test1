...
class TypeProduitController extends AbstractController
{
    ...
    /**
     * @Route("/{type_id}", name="type_produit_show", methods={"GET"})
     */
    ...
    /**
     * @Route("/{type_id}/edit", name="type_produit_edit", methods={"GET","POST"})
     */
    ...
    /**
     * @Route("/{type_id}", name="type_produit_delete", methods={"DELETE"})
     */
    public function delete(Request $request, TypeProduit $typeProduit): Response
    {
        if ($this->isCsrfTokenValid('delete'.$typeProduit->getType_id(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
